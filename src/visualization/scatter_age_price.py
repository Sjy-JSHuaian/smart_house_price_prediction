"""
scatter_age_price.py

绘制加州房价房龄与房价的散点图，并保存到 reports/figures/。
"""
...
plt.title("House Age vs. House Value in California")
...
plt.savefig(
    output_dir / "scatter_age_price.png",
    dpi=300,
    bbox_inches="tight",
)
...
# ── 输出相关系数 ─────────────────────────────────────────────
corr = np.corrcoef(df["HouseAge"], df["MedHouseVal"])[0, 1]
print(f"皮尔逊相关系数 (HouseAge vs MedHouseVal): {corr:.4f}")
print(f"图片已保存至: {output_dir / 'scatter_age_price.png'}")
